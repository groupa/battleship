...
public class ScreenDisplayer extends BackgroundPanel implements
		MouseInputListener {
	...
	/**
	 * Constructor for screendisplayer: mouselistener added, counter value for
	...
	/**
	 * Displays Game Finished screen. Receives one argument (int) that controls
	 * if it displays win or loose. 0=win, anything else is loose.
	 */
	...
	/*
	 * This method must be called at each iteration of the game play loop. This
	 * method updates the graphic 20frames per second game loop can never stop,
	 * or else the animations flow is at risk.
	 */
	public void updateGraphics() {
		...
		if (smallXplsn.isVisible()) {
			...
			smallXplsn.setIcon(new ImageIcon(smallXplsnBuff.getSubimage(
					counter[0] * 34, counter[1] * 34, 34, 34)));
			counter[0]++;
			if (counter[1] == 3 && counter[0] == 3) {
				if (dBg)
					System.out.println("Small Explosion Animation Finished");
				smallXplsn.setVisible(false);
				smallXplsn.setIcon(new ImageIcon(smallXplsnBuff.getSubimage(0,
						0, 34, 34)));
				counter[0] = 0;
				counter[1] = 0;
		...
		if (bigXplsn.isVisible()) {
			...
			bigXplsn.setIcon(new ImageIcon(bigXplsnBuff.getSubimage(
					counter[2] * 64, counter[3] * 64, 64, 64)));
			counter[2]++;
			if (counter[3] == 3 && counter[2] == 3) {
				if (dBg)
					System.out.println("Big Explosion Animation Finished");
				bigXplsn.setVisible(false);
				bigXplsn.setIcon(new ImageIcon(bigXplsnBuff.getSubimage(0, 0,
						64, 64)));
				counter[2] = 0;
				counter[3] = 0;
			} else if (counter[2] == 3) {
		...
		if (missAni.isVisible()) {
			...
			missAni.setIcon(new ImageIcon(missAniBuff.getSubimage(
					counter[4] * 34, counter[5] * 34, 34, 34)));
			counter[4]++;
			if (counter[5] == 3 && counter[4] == 3) {
				if (dBg)
					System.out.println("miss Animation Finished");
				missAni.setVisible(false);
				missAni.setIcon(new ImageIcon(missAniBuff.getSubimage(0, 0, 34,
						34)));
				counter[4] = 0;
				counter[5] = 0;
		...
		// this block controls the frame rate. (how fast the game updates) you
		// can modify the value here.
		try {
	 ...
	 * indx = what cell index board = on which board.
	 */
	public void hitShot(int indx, int board) {
		smallXplsn.setLocation(40 + ((indx % 10)) * 34 + 380 * (board - 1),
				220 + ((indx / 10)) * 34);
		smallXplsn.setVisible(true);
		playHit();
		if (dBg)
			System.out.println("small explosion located in x: "
					+ (40 + ((indx % 10)) * 34 + 380 * (board - 1)) + " y: "
					+ (220 + ((indx / 10)) * 34));
	}
	...
	public void missShot(int indx, int board) {
		missAni.setLocation(40 + ((indx % 10)) * 34 + 380 * (board - 1),
				220 + ((indx / 10)) * 34);
		missAni.setVisible(true);
		playMiss();
		if (dBg)
			System.out.println("Miss animation located in x: "
					+ (40 + ((indx % 10)) * 34 + 380 * (board - 1)) + " y: "
					+ (220 + ((indx / 10)) * 34));
	}

	/**
	 * Activates the ship-destroyed-related methods (animation, sound,
	 * indication) indx = what cell index board = on which board.
	 */
	public void destroyShot(int indx, int board) {
		bigXplsn.setLocation(8 + ((indx % 10)) * 34 + 380 * (board - 1),
				188 + ((indx / 10)) * 34);
		bigXplsn.setVisible(true);
		playDestroyed();
		if (dBg)
			System.out.println("Big explosion located in x: "
					+ (8 + ((indx % 10)) * 34 + 380 * (board - 1)) + " y: "
					+ (188 + ((indx / 10)) * 34));
	}
	...
	private void showHit(int x, int y) {
		...
		hits.get(hits.size() - 1).setBounds(x, y, image.getIconWidth(),
				image.getIconHeight());
	...
	private void showMiss(int x, int y) {
		...
		misses.get(misses.size() - 1).setBounds(x, y, image.getIconWidth(),
				image.getIconHeight());
	...
	/**
	 * Exits the program. Invoked when credits-screen is entered. 5 seconds
	 * after the game exits, giving the user time to read.
	 */
	private void exit() {
		try {
	...
	/**
	 * If statements for checking if any, and which label is pressed, then
	 * changes it appearance
	 */
	public void mousePressed(MouseEvent e) {
		...
		if (layeredPane.findComponentAt(e.getX(), e.getY()) == InstructionsSmallText) {
			InstructionsSmallText.setIcon(new ImageIcon(
					"img/Instructionspush.png"));
		}
		...
		if (layeredPane.findComponentAt(e.getX(), e.getY()) == PlayerVsPlayer) {
			PlayerVsPlayer.setIcon(new ImageIcon(
					"img/playervsplayertextpush.png"));
		}
		if (layeredPane.findComponentAt(e.getX(), e.getY()) == TwoPlayerVsTwoPlayer) {
			TwoPlayerVsTwoPlayer.setIcon(new ImageIcon(
					"img/2playervs2playertextpush.png"));
		}
		...
		if (layeredPane.findComponentAt(e.getX(), e.getY()) == returnToMain) {
			returnToMain.setIcon(new ImageIcon(
					"img/return to main menu push.png"));
		}
	...
	public void mouseReleased(MouseEvent e) {
		...
		if (layeredPane.findComponentAt(e.getX(), e.getY()) == ExitGameText) {
			...
			if (dBg)
				System.out
						.println("Game exits after 5seconds delay, and credit screen displayed");
		}
		if (layeredPane.findComponentAt(e.getX(), e.getY()) != ExitGameText) {
	...
	/**
	 * If statements to see if any labels are hoovered over. If so, the image is
	 * changed. If left, the image is changed.
	 */
	public void mouseMoved(MouseEvent e) {
		...
		if (layeredPane.findComponentAt(e.getX(), e.getY()) == PlayerVsPlayer) {
			PlayerVsPlayer.setIcon(new ImageIcon(
					"img/playervsplayertextholdover.png"));
		}
		if (layeredPane.findComponentAt(e.getX(), e.getY()) != TwoPlayerVsTwoPlayer) {
			TwoPlayerVsTwoPlayer.setIcon(new ImageIcon(
					"img/2playervs2playertext.png"));
		...
		if (layeredPane.findComponentAt(e.getX(), e.getY()) == TwoPlayerVsTwoPlayer) {
			TwoPlayerVsTwoPlayer.setIcon(new ImageIcon(
					"img/2playervs2playertextholdover.png"));
		}
		if (layeredPane.findComponentAt(e.getX(), e.getY()) != Easy) {
			Easy.setIcon(new ImageIcon("img/easytext.png"));
		}
		...
		if (layeredPane.findComponentAt(e.getX(), e.getY()) == returnToMain) {
			returnToMain.setIcon(new ImageIcon(
					"img/return to main menu holdover.png"));
		}
		if (layeredPane.findComponentAt(e.getX(), e.getY()) != mainMenu) {
			mainMenu.setIcon(new ImageIcon("img/mainmenu.png"));
		}
	...
	/**
	 * Set method for ship orientation. The index of the ships location and
	 * which ship
	 * 
	 * @param ships
	 *            = the indexes that the ships overlaps.
	 * @param ship
	 *            = which ship
	 * @param horVar
	 *            = horizontal or vertical (vertical = 1, horizontal =
	 *            everything else)
	 * @param board
	 *            = which board
	 */
	public void setShipOrientation(ArrayList<Integer> ships, int ship,
			int horVar, int board) {
		if (board == 1) {
			if (horVar != 1)
				switch (ship) {
				case 1:
					ship1.setBounds(40 + ((ships.get(0) % 10)) * 34,
							220 + ((ships.get(0) / 10)) * 34, ship1.getWidth(),
							ship1.getHeight());
					if (dBg)
						System.out.println("Set ship1 horizontal on the left board");
					break;
				case 2:
					ship2.setBounds(40 + ((ships.get(0) % 10)) * 34,
							220 + ((ships.get(0) / 10)) * 34, ship2.getWidth(),
							ship2.getHeight());
					if (dBg)
						System.out.println("Set ship2 horizontal on the left board");
					break;
				case 3:
					ship3.setBounds(40 + ((ships.get(0) % 10)) * 34,
							220 + ((ships.get(0) / 10)) * 34, ship3.getWidth(),
							ship3.getHeight());
					if (dBg)
						System.out.println("Set ship3 horizontal on the left board");
					break;
				case 4:
					ship4.setBounds(40 + ((ships.get(0) % 10)) * 34,
							220 + ((ships.get(0) / 10)) * 34, ship4.getWidth(),
							ship4.getHeight());
					if (dBg)
						System.out.println("Set ship4 horizontal on the left board");
					break;
				case 5:
					ship5.setBounds(40 + ((ships.get(0) % 10)) * 34,
							220 + ((ships.get(0) / 10)) * 34, ship5.getWidth(),
							ship5.getHeight());
					if (dBg)
				...
				case 1:
					...
					tmpC = ship1.getWidth();
					ship1.setBounds(40 + ((ships.get(0) % 10)) * 34,
							220 + ((ships.get(0) / 10)) * 34, tmp, tmpC);
			...
			if (horVar != 1)
				switch (ship) {
				case 1:
					enemyShip1.setBounds(40 + ((ships.get(0) % 10)) * 34,
							220 + ((ships.get(0) / 10)) * 34, enemyShip1
									.getWidth(), enemyShip1.getHeight());
					if (dBg)
						System.out.println("Sets enemyShip1 horizontal on the right board");
					break;
				case 2:
					enemyShip2.setBounds(40 + ((ships.get(0) % 10)) * 34,
							220 + ((ships.get(0) / 10)) * 34, enemyShip2
									.getWidth(), enemyShip2.getHeight());
					if (dBg)
						System.out.println("Sets enemyShip2 horizontal on the right board");
					break;
				case 3:
					enemyShip3.setBounds(40 + ((ships.get(0) % 10)) * 34,
							220 + ((ships.get(0) / 10)) * 34, enemyShip3
									.getWidth(), enemyShip3.getHeight());
					if (dBg)
						System.out.println("Sets enemyShip3 horizontal on the right board");
					break;
				case 4:
					enemyShip4.setBounds(40 + ((ships.get(0) % 10)) * 34,
							220 + ((ships.get(0) / 10)) * 34, enemyShip4
									.getWidth(), enemyShip4.getHeight());
					if (dBg)
						System.out.println("Sets enemyShip4 horizontal on the right board");
					break;
				case 5:
					enemyShip5.setBounds(40 + ((ships.get(0) % 10)) * 34,
							220 + ((ships.get(0) / 10)) * 34, enemyShip5
									.getWidth(), enemyShip5.getHeight());
					if (dBg)
				...
				case 4:
					...
					if (dBg)
						System.out
								.println("Sets enemyShip4 vertical on the right board");
					break;
				case 5:
					tmp = enemyShip5.getHeight();
